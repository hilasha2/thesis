...
pcl.read <- function(datafile, metadata.rows = NA, rowfeatures=T, sep="\t") {
    ...
    if (endsWith(datafile, ".gz")) {
        ...
        raw <- read.csv(datafile, stringsAsFactors=F, sep="\t", comment.char="")
    }
    # taking the first columm which consists sample names from the metadata and features from the data.
    featnames <- raw[,1]
...
pcl.read.mine <-function(data.path, metadata.path, file.type, rowfeatures = T) {
  # data.path - path to file result from biobakery.
  # metadata.path - path to human microbiome project 2's metadata.
  # file.type - taxonomy/humann/no_change etc. - 
  #             which type of biobakery result file we are inputing 
  #             if it needs sample ID renaming. 
  # rowfeatures - whether the features are in the rows of the data.
  # returns a list with
  ...
  #metadata <- as.data.frame(t(metadata), stringsAsFactors=F)
  if (rowfeatures) {
    # Transpose if rows are features in the data
  ...
  } else if(file.type == "humann") {
    rownames(data) <- str_remove(rownames(data), "_Abundance-RPKs")
  } else if(file.type == "no_change") {
    print("No need to rename sample IDs in the data file")
  } else {
...
pcl.heatmap <- function(dat, ..., meta = F, logspace = F, pseudocount = 0,
    ...
    #neg <- any(dat$x[sapply(dat$x, is.finite)] < 0)
    if (is.na(divergent0)) {
        divergent0 <- neg
    }
    ...
    def.params$clustering_distance_rows <- override_clustering(x,
        def.params$clustering_distance_rows, def.params$clustering_distance_rows)
    def.params$clustering_distance_cols <- override_clustering(t(x),
        def.params$clustering_distance_cols, def.params$clustering_distance_cols)
    ...
    if (reorder) {
        def.params$clustering_callback <- function(hc, u) {
            ...
            m <- if (all(dim(u) == dim(x)) && all(u == x)) {
                def.params$clustering_distance_rows
            } else {
                def.params$clustering_distance_cols
            }
...
#     }
    ...
    library(randomcoloR)
    participants.list <- dat$meta$Participant.ID
    unique.participants <- unique(participants.list)
    def.params$annotation_col['participant'] <- factor(participants.list, levels = unique.participants)
    dist_colors <- distinctColorPalette(length(unique.participants))
    #params$annotation_colors$participant <- purrr::map_chr(participants.list, 
    #                                                ~dist_colors[which(unique(participants.list) == .x)])
    temp.df <- data.frame(matrix(ncol = length(unique.participants), nrow = 0))
    colnames(temp.df) <- unique.participants
    temp.df[1, ] <- dist_colors
    params$annotation_colors$participant <- unlist(as.list(temp.df), use.names = T)
    params$annotation_legend <- F
    params$fontsize <- 12
    
    
    args <- c(list(mat=x), params, def.params)
    args <- args[unique(names(args))]
    ff <- do.call(pheatmap, args)
    typeof(ff)
}
