#setwd("~/analysis/hmp2_multiomics_paper")
...
ordination_battery <- function(pcl, name) {
    ...
    do_plots <- function(pcl, subname) {
        ...
        png(sprintf("./overview/%s_%s_heatmap_top30_ab.png", name, subname), 1700, 850)
        #pdf(sprintf("./overview/%s_%s_heatmap_top30_ab.pdf", name, subname), 18, 7, onefile=F)
        make_heatmap(pcl %>% pcl.top.f(mean(x), n=30))
        dev.off()
        #ff <- make_heatmap(pcl %>% pcl.top.f(mean(x), n=30))

        png(sprintf("./overview/%s_%s_heatmap_top30_abwhenpres.png", name, subname), 1700, 850)
        ...
        if (all(c("diagnosis", "site_sub_coll") %in% colnames(pcl$meta))) {
            png(sprintf("./overview/%s_%s_heatmap_top30_ab_nohclust.png", name, subname), 1700, 850)
        ...
        png(sprintf("./overview/%s_%s_heatmap_top30_pr.png", name, subname), 1700, 850)
        ...
        if (all(c("diagnosis", "site_sub_coll") %in% colnames(pcl$meta))) {
            print("#####nohlcust")
            png(sprintf("./overview/%s_%s_heatmap_top30_pr_nohclust.png", name, subname), 1700, 850)
    ...
    if (pcl.only(pcl, rank="s")$nf > 0) {
        ...
        print("####### working on genus rank now:")
...
# 21_10_21 - I am converting bugs.pcl$x to be a numeric and not a data.frame, otherwise, functions don't work here.
#bugs.pcl2 <- lapply(bugs.pcl, copy)
#bugs.pcl2$x <- data.matrix(bugs.pcl$x)
# 11_11_11 - I changed pcl.read.mine to return a matrix instead of data.frame.
...
################### start here for pcoa plots - 06 July 2023

plot_pcoa <- function(bugs.pcl, plot_name) {
  ...
  w <- 0.23
  sq <- 0.043
  pdf(sprintf("./overview/overview_taxonomy_ordinations_%s.pdf", plot_name), 7.5, 6)
  print(ggdraw() +
  ...
  dev.off()
  svg(sprintf("./overview/overview_taxonomy_ordinations_%s.svg", plot_name), 7.5, 6)
  print(ggdraw() +
          draw_plot(so.main,   0,   0, 1-w, 1) +
          draw_plot(so.gini,  1-w,  sq,   w, (1-sq)/3) +
          draw_plot(so.firm, 1-w, sq+(1-sq)/3,   w, (1-sq)/3) +
          draw_plot(so.bact, 1-w, sq+2*(1-sq)/3,   w, (1-sq)/3))
  dev.off()
  # it works if I take it outside the function
  pdf(sprintf("./overview/overview_taxonomy_ordinations_legends_%s.pdf", plot_name), 7, 5)
...
species.ord <- plot_pcoa(bugs.pcl, "hmp2_mgx_metaphlan3_06_July_2023")
species.ord2 <- plot_pcoa(bugs.pcl2, "hmp2_mgx_metaphlan2_06_July_2023")

library(vegan)
vare.proc <- procrustes(species.ord, species.ord2)
scores <- plot(vare.proc, kind=1, type=NULL)
protest(species.ord, species.ord2)
############## end pcoa plot - 06 July 2023
...
plot_alpha_divs <- function(pcl, name) {
    library(ggsignif)
    library(cowplot)
    # returns a list of the size of ns
    ...
    add_plot <- function(y.aes, y.ttl, ggp_base, df) {
      p <- ggp_base + geom_boxplot(aes(y=y.aes), size=0.5, outlier.size=1) +
        ylab(y.ttl) + 
        geom_signif(aes(y=y.aes), 
                    comparisons = list(c("nonIBD", "UC"), 
                                       c("nonIBD", "CD"),
                                       c("UC", "CD")),
                    map_signif_level = T,
                    step_increase =  0.1,
                    test = "wilcox.test")
      kw <- kruskal.test(y.aes ~ diagnosis, data = df)
      corr.label <- str("p = %g", kw$p.value)
    ...
    print(add_plot(df$gs, "Gini-Simpson", ggp_base, df))

    print(add_plot(df$is, "Inverse Simpson", ggp_base, df))

    print(add_plot(df$sh, "Shannon Index (bits)", ggp_base, df))
    return(df)
...
# 28_Jan_2022
bugs.species <- pcl.only(bugs.pcl, rank = "s")
# Finding which samples (external id) were the first sample (min of week_num) of each participant
df.unique.participants <- bugs.species$meta %>% group_by(Participant.ID) %>% 
  dplyr::summarise(min_idx = which.min(week_num), 
                  first_week = min(week_num), 
                 external.id = External.ID[which.min(week_num)]) 
bugs.species.unique <- list()
bugs.species.unique$x <- bugs.species$x[df.unique.participants$external.id, ]
bugs.species.unique$meta <- bugs.species$meta[df.unique.participants$external.id, ]
bugs.species.unique$ns <- dim(bugs.species.unique$x)[1]
bugs.species.unique$nf <- dim(bugs.species.unique$x)[2]
alpha.div.df <- plot_alpha_divs(bugs.species.unique, "species_unique_participants")
...
#bar <- bugs.pcl$meta %>% group_by(Participant.ID) %>% 
 #  dplyr::summarise(min_idx = which.min(week_num), 
  #                  first_week = min(week_num), 
   #                 external.id = External.ID[which.min(week_num)]) 
